...
Revision 3 â€” supersedes the sequence in the Implementation Directive's "Build Order".
...
what order; that one says why, and marks which edges are real dependencies versus Â§13 ordering.

This is an implementation plan, not architecture. Spec v1.4 Â§13's *binding* order is unchanged
and unchangeable: universal infrastructure â†’ Affiliate â†’ Finance Tracking â†’ plugin framework â†’
Trading Analysis â†’ additional types â†’ live trading. What changes below is only how the work is
...
**Was:** M4 = Affiliate Business (Â§13 Step 2).
...
the thing that *wakes up and decides*. Spec Â§2.1's Business Manager is that thing â€” it is the
caller for scheduled work, for concurrent capability dispatch, and for the approval lifecycle.

The original plan buried the Business Manager inside the Affiliate Business milestone. But Â§2.1
makes the Manager a per-business Temporal workflow whose *pattern* is platform infrastructure,
and Â§4.1 requires that pattern "be treated as the general pattern for any future nesting"
...
Â§13 Step 1's own wording supports the split: universal infrastructure is "built against one real
...
pattern presents as a defect in the Affiliate business, and vice versa â€” two unfamiliar systems
failing together, with no way to bisect.

**It makes the first business type a real test of Â§4.** Â§4 requires a new instance of a known
type be addable "via configuration only â€” no orchestrator code changes". If the Manager is built
...
visibly is not â€” which is exactly the signal Â§4 exists to produce.

**It surfaces the district-readiness constraint while it is still cheap.** Â§3.2 and Â§4.1 require
...
the scope Â§2.1 gives it â€” tactical execution, never strategy â€” and Â§3.1's Executive
responsibilities remain untouched and unbuilt. The hierarchy in Â§3.2 is unchanged. This is a
regrouping of implementation work inside Â§13's binding order.
...
| Nothing starts a Manager workflow | An ACTIVE business has no Manager, violating Â§2.1 |
...
**Why the new ordering better reflects the implementation.** Activation is platform work â€” every
business type needs it, and none of it is Affiliate-specific. Extracting it means M6 is a
configuration exercise, which is the only condition under which Â§4's "addable via configuration
only â€” no orchestrator code changes" is testable. This is the same argument as revision 2,
applied consistently: a component every instance needs does not belong inside the first instance.

It also retires two deferred-completion rows rather than opening more â€” `CredentialManager` gains
...
|---|---|---|---|
| 1 | Platform Kernel foundation | Â§0, Â§0.1, Â§5, Â§11, Â§11.5 | Merged |
| 2 | Execution spine | Â§2, Â§2.2, Â§6, Â§9, Â§10 | Merged |
| 3 | Operator surface | Â§5, Â§8, Â§9, Â§12.5 | Merged |
| 4 | Business Manager runtime + scheduler | Â§2.1, Â§2.2, Â§9 | Merged |
| 5 | **Business Activation Path** | Â§2.1, Â§4, Â§10, D-006 | **Current** |
| 6 | Affiliate Business | Â§13 Step 2 | Next |
| 7 | Finance Tracking Business | Â§13 Step 3 | Planned |
| 8 | Plugin framework | Â§13 Step 4, Â§4 | Planned |
| 9 | Executive Layer | Â§3, Â§3.1 | Planned |
| 10 | Trading Analysis Business | Â§13 Step 5 | Planned |
| 11 | Additional business types | Â§13 Step 6 | Planned |
| 12 | Live Trading Business | Â§13 Step 7, Â§8 | Planned |

Milestone 9 (Executive Layer) sits after the plugin framework because Â§13 Step 4 generalises
"once two business types exist for real comparison", and Â§3.1's cross-business responsibilities â€”
capital allocation, portfolio balancing â€” are meaningless with fewer than two businesses to
allocate between. This placement was implicit in Â§13 and is now explicit.
...
2. **Dependency graph delta** â€” new or changed edges, each classified Hard, Evidential,
   Â§13 ordering, or Soft, plus any deferred-completion rows opened or retired
3. Objective
...
memory. A milestone that opens more deferred-completion rows than it retires should say why â€”
...
**"Surfaced in the Shell"** â€” what the milestone's features look like in the running
...
layering invariant, which requires that decision be made deliberately â€” this is it, and
`test_composition_roots_hold_no_logic` keeps the root free of behaviour.

**Per-milestone surfacing plan:** M6 â€” create a Finance company beside the Affiliate one; watch
a Manager wake live. M7 â€” installed templates page. M8 â€” Executive view: budget moves between
companies, explained. M9+ â€” trading analysis appears read-only long before anything can act.

---

## Revision 4: prove the platform with a vertical slice before widening it (structural)

**What changed.** M6 was "Finance Tracking â€” the second business type." It is now **M6: the
...
and is unit-tested in isolation â€” Manager, capabilities, approval gate, tool execution, audit
trail â€” but no single company has yet travelled the whole path:

```
create company â†’ Manager wakes â†’ business executes a capability â†’
action proposed â†’ approval generated â†’ operator approves â†’
tool executes the approved action â†’ audit + decision trail recorded
```
...
**Why the new ordering is better.** Proving one complete slice validates the platform itself â€”
...
Prove the platform, then widen it â€” not the reverse.

**Classification: structural.** No architecture changes â€” no layer, responsibility, or invariant
...
first live run where it belongs â€” as the spine of the slice rather than a separate task beside
a new business type.
...
| 3 | Operator surface | Â§5, Â§8, Â§9, Â§12.5 | Merged |
| 4 | Business Manager runtime + scheduler | Â§2.1, Â§2.2, Â§9 | Merged |
| 5 | Affiliate Business (definition + provisioning) | Â§13 Step 2 | Merged |
| â€” | Developer Shell + desktop application | operational | Merged |
| 6 | Affiliate vertical slice â€” prove the platform end to end | Â§2.1, Â§6, Â§8, Â§13 | Merged (report: `docs/reports/M6.md`) |
| 7 | Finance Tracking Business | Â§13 Step 3 | Merged (report: `docs/reports/M7.md`) |
| 8 | **Plugin framework** | Â§13 Step 4, Â§4 | **Next** |
| 9 | Executive Layer | Â§3, Â§3.1 | Planned |
| 10 | Trading Analysis Business | Â§13 Step 5 | Planned |
| 11 | Additional business types | Â§13 Step 6 | Planned |
| 12 | Live Trading Business | Â§13 Step 7, Â§8 | Planned |
...
1. **Platform correctness** â€” does the right thing, safely.
2. **Complete vertical slices** â€” whole paths work end to end.
3. **Workflow refinement** â€” the operator's tasks are smooth.
4. **Product experience refinement** â€” it becomes genuinely good to use.
5. **Commercial-quality polish** â€” it becomes premium.

Correctness always comes first. But once correctness exists, **product quality is a first-class
engineering objective, not an optional enhancement** â€” it has a defined place in the sequence,
...
The full product constitution â€” objective, philosophy, inspiration, governance â€” is
...
and a complete slice â€” tiers 1 and 2). Product review begins gating now that surfaces exist, and
